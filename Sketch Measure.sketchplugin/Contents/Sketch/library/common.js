    ...
    getExportable: function (layer, savePath) {
        ...
        var exportFormats =
            (self.configs.unit == "dp/sp" && matchFormat) ? [{
                    scale: 1 / self.configs.scale,
                    prefix: "drawable-mdpi/",
                    format: fileFormat
                },
                {
                    scale: 1.5 / self.configs.scale,
                    prefix: "drawable-hdpi/",
                    format: fileFormat
                },
                {
                    scale: 2 / self.configs.scale,
                    prefix: "drawable-xhdpi/",
                    format: fileFormat
                },
                {
                    scale: 3 / self.configs.scale,
                    prefix: "drawable-xxhdpi/",
                    format: fileFormat
                },
                {
                    scale: 4 / self.configs.scale,
                ...
                while (tempSymbolLayer = tempSymbolLayers.nextObject()) {
                    ...
                    if (self.is(tempSymbolLayer, MSSymbolInstance)) {
                        ...
                        if (
                            ...
                            }, self.document.documentData().allSymbols());
                            if (changeSymbol) {
                    ...
                    if (!exporting) {
                        ...
                        if (layerIndex >= artboard.children().length) {
                            ...
                            if (!self.configs.exportOption) {
                                ...
                                self.writeFile({
                                    content: self.template(template, {
                                        lang: language,
